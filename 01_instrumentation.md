---
title: Instrumentation
---

## Ultramicrotome
The essential components of a microtome are: a sample mounted in/on a bullet of epoxy; a chuck to secure the bullet; a knife blade; a mechanism to advance the sample towards the knife, and raise and lower the sample vertically at the knife edge.
Automated ultramicrotomes advance the specimen arm holding the embedded sample block towards the cutting knife edge with precision and reproducibility. 
The knife stage maintains the cutting edge at adjustable clearance angles, typically 6°, optimized for the specimen's properties. A stereomicroscope with appropriate illumination permits real-time monitoring of the sectioning process. The sample slices float off of the knife into a bath, usually filled with water.
Either glass or diamond knives can be used, depending on the materials being cut and the desired thickness. 


:::{figure} ./figures/fig_1.png
:name: labeled_ultramicrotome
**Components of an ultramicrotome for electron microscopy sample preparation.** The ultramicrotome consists of key components including the arm/sample chuck which moves the embedded sample block toward the diamond or glass knife mounted in the knife holder. The knife boat (attached to diamond knife) has a water bath that contains ultrapure water to collect thin sections as they are cut. Progress is seen through a stereomicroscope that provides magnified visualization of the sectioning process, allowing the operator to monitor section quality and manipulate ribbons. The microtome stage advance mechanism controls the feed rate to produce sections suitable for transmission electron microscopy. The instrument base is mounted on an anti-vibration table to minimize chatter artifacts during sectioning.
:::

### Mechanical Advance System: 
The mechanical system controls the sectioning interval and determines section thickness. It includes a drive wheel and knife wheel that precisely advance the sample toward the knife. Modern ultramicrotomes use mechanical advance systems, while older models used thermal expansion where heating an arm caused it to lengthen and advance the sample. This allows for personally controlled movements necessary for rough cutting.


:::{figure} ./figures/fig_2.png
:name: rotary_wheel
**Rotary wheel mechanism of the ultramicrotome.** The rotary wheel (handwheel) provides manual control of the specimen arm advance, allowing the operator to move the specimen block toward the knife in precise increments. The wheel is used for initial trimming, specimen alignment, and positioning adjustments during the sectioning process. The "7" position on the wheel indicates the point where the knife edge contacts and cuts through the specimen block.
:::



### Knife and Knife Holder: 
The ultramicrotome uses either a glass knife for initial rough cuts or diamond knife for most ultra-thin sectioning. 
Diamond knives come in different angles, typically 35°or 45°, with edge lengths ranging from 1 to 7 mm depending on the application. 
The knife has an attached boat or trough that holds water, allowing the sections to float as they are cut.

:::{figure} ./figures/fig_3.png
:name: microtome_knives 
**Ultramicrotome configurations with glass and diamond knives.** (A) Ultramicrotome fitted with a glass knife, offering a cost-effective, disposable alternative suitable for routine sectioning with lower volumes. (B) Ultramicrotome equipped with a diamond knife, which provides extended cutting life and consistent performance for high-throughput serial sectioning of hundreds of thousands of sections. Diamond configuration includes a boat for collecting ultrathin sections (50-100 nm) in ultrapure water.
:::

### Sample Holder (Chuck): 
The sample holder secures the embedded specimen block and includes a built-in system for orienting the sample surface relative to the knife blade. 
Different types of chucks accommodate samples embedded in various molded shapes (generally pyramidal).

:::{figure} ./figures/fig_4.png
:name: sample_chuck
**Mounting the specimen chuck into the ultramicrotome specimen arm.** The specimen chuck, containing the embedded sample block, is securely inserted into the specimen arm holder of the ultramicrotome. Proper mounting ensures stable specimen positioning and prevents vibration during sectioning. The chuck is aligned and tightened to maintain the correct orientation of the sample block relative to the knife edge, with the block face positioned perpendicular to the cutting direction for optimal sectioning quality.
:::


### Stereomicroscope: 
A binocular stereomicroscope is mounted on the ultramicrotome to view the knife holder and block in three dimensions during setup and sectioning.

### Control System: 
Modern ultramicrotomes have controls for adjusting cutting speed, section thickness, and positioning. 
The operator must manage six degrees of freedom: X and Y axes of the knife holder, rotational angle of the knife holder, clearance angle of the knife holder, and the pitch and rotation of the block.

### Stage: 
The base platform holds and positions the sample holder, allowing for precise trimming and sectioning operations.




## Micromanipulator

:::{figure} ./figures/fig_5.png
:name: fig_5
**Micromanipulator for precision specimen manipulation.** The Microsupport Axis Pro micromanipulator provides multi-axis control for precise positioning and manipulation of specimens during microscopy and sample preparation. The device features independent X, Y, and Z-axis adjustment with fine-scale control, allowing operators to achieve micrometer-level positioning accuracy. 
:::


### Heated Quartz Plate

:::{figure} ./figures/fig_6a.png
:name: removable_stage
:::

:::{figure} ./figures/fig_6b.png
:name: stage_mounted
**Custom removable stage for sulfur embedding.** A specialized stage was fabricated with increased mass for vibration dampening, thermal isolation to prevent external heat transfer, and precise leveling mechanisms. Isolation mounts decouple the working surface from building vibrations and mechanical disturbances, ensuring stable conditions during the critical sulfur solidification phase and precise specimen positioning.
:::